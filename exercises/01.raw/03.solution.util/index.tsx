...
function use<Value>(promise: Promise<Value>): Value {
	const hackyPromise = promise as Promise<Value> & {
		status: string
		value: Value
		reason: any
	}
	if (hackyPromise.status === 'fulfilled') {
		return hackyPromise.value
	} else if (hackyPromise.status === 'rejected') {
		throw hackyPromise.reason
	} else if (hackyPromise.status === 'pending') {
		throw hackyPromise
	} else {
		hackyPromise.status = 'pending'
		hackyPromise.then(
			result => {
				hackyPromise.status = 'fulfilled'
				hackyPromise.value = result
			},
			reason => {
				hackyPromise.status = 'rejected'
				hackyPromise.reason = reason
			},
		)
		throw hackyPromise
	}
