...
The core of everything we'll be doing revolves around the `Suspense` component
and the `use` hook. The `Suspense` component enables you to declaratively tell
React what to render while waiting for something to load. The `use` hook enables
you to convert something async (a promise) into the resolved value.
